...
# recupération du nom du fichier SEQUOIA_TRIM_REGION_BRUT qui varie dans l'archive zip  
sequoia_trim_region <- grep(
  "BRUT", 
  unzip(zipfile = "data/SEQUOIA_TRIM_REGION.zip", list = TRUE)$Name, 
  value = TRUE
)
unzip(zipfile = "data/SEQUOIA_TRIM_REGION.zip", exdir = "data")
SEQUOIA <- read_excel(file.path("data", sequoia_trim_region), sheet = "PAYS_DE_LA_LOIRE")
datatable(SEQUOIA)
```
Exemple en utilisant l'api de l'URSSAF :

```{r lire_2b, eval=FALSE}
...
urssaf_fr <- fromJSON(url_urssaf_fr)$records$fields
datatable(urssaf_fr)
```
