...
Dans une opération de filtre, on n'utilise pas (plus !) `across()` mais, `if_all` ou `if_any` pour préciser si l'on souhaite que le critère de filtre s'applique à toutes les colonnes qui remplissent la condition de sélection de colonnes, ou si seulement une colonne suffit : 

```{r}
filter(parametre, if_any(.cols = starts_with("date"), .fns = ~ .x > "2015-01-01")) %>%
  select(1:7)
```

référence : <https://dplyr.tidyverse.org/reference/across.html>
